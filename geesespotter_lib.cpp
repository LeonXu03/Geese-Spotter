...
void startGame(char * & board, std::size_t & xdim, std::size_t & ydim, unsigned int & numgeese)
{
  ...
  cleanBoard(board);  //deallocate and remove the board from dynamically allocated memory
  board = createBoard(xdim, ydim);  //create the board
  printBoard(board, xdim, ydim);    
  spreadGeese(board, xdim, ydim, numgeese);
  mark(board, xdim, ydim, 0, 0);
